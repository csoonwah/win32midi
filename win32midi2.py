...
def midiOutGetDevCapsA(uDeviceID, lpMidiOutCaps):
    ...
    #lpMidiOutCaps.szPname = ctypes.cast(lpMidiOutCaps.szPname,ctypes.c_char_p)
    return _wm.midiOutGetDevCapsA(uDeviceID, byref(lpMidiOutCaps), ctypes.sizeof(lpMidiOutCaps))
...
def midiInGetDevCapsA(uDeviceID, lpMidiOutCaps):
    return _wm.midiInGetDevCapsA(uDeviceID, byref(lpMidiOutCaps), ctypes.sizeof(lpMidiOutCaps))
...
def midiOutPrepareHeader(hmo, lpMidiOutHdr):
    return _wm.midiOutPrepareHeader(hmo, byref(lpMidiOutHdr), ctypes.sizeof(lpMidiOutHdr))

def midiOutLongMsg(hmo, lpMidiOutHdr):
    return _wm.midiOutLongMsg(hmo, byref(lpMidiOutHdr), ctypes.sizeof(lpMidiOutHdr))

def midiOutUnprepareHeader(hmo, lpMidiOutHdr):
    return _wm.midiOutUnprepareHeader(hmo, byref(lpMidiOutHdr), ctypes.sizeof(lpMidiOutHdr))
...
def midiStreamOpen(puDeviceID, cMidi, dwCallback, dwInstance=0, fdwOpen=0):
    ...
    return _wm.midiStreamOpen(lphStream, puDeviceID, cMidi, dwCallback, dwInstance, fdwOpen), lphStream
...
def midiStreamOut(hMidiStream, lpMidiHdr):
    return _wm.midiStreamOut(hMidiStream, byref(lpMidiHdr), ctypes.sizeof(lpMidiHdr))
